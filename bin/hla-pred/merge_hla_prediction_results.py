        ...
        with open(file_path, 'r') as file:
            ...
            for line in lines:
                if line.startswith("   1 HLA-"):
                    ...
                    if not ((merged_df['peptide'] == peptide) & (merged_df['allele'] == allele)).any():
                        # Add a new row if it doesn't exist
                        #merged_df = merged_df.append({'peptide': peptide, 'allele': allele, 'NetMHCpan_%Rank': perc_rank}, ignore_index=True)
                        new_row = pd.DataFrame([{'peptide': peptide, 'allele': allele, 'NetMHCpan_%Rank': perc_rank}])
                        merged_df = pd.concat([merged_df, new_row], ignore_index=True)
                    else:
                        # Update the existing row
                        merged_df.loc[(merged_df['peptide'] == peptide) & (merged_df['allele'] == allele), 'NetMHCpan_%Rank'] = perc_rank
