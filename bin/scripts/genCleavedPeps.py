...
# TODO remove this function once sure it won't be used
# def getIDsFromGff(genesToExcludeFromReference: str, gff: str):
    
#     excludeGenes = set()
#     with open(genesToExcludeFromReference) as txt:
#         for line in txt:
#             excludeGenes.add(line.strip())
...
#                         if name in excludeGenes:
#                             allIDs.add(desc.split(';')[0].split(':')[1])
#     return allIDs
...
def buildReferenceSet(referenceFasta, pepLens=[8, 9, 10, 11], cleaveFile=None, cleavageThreshold=1.0, gff=None, genesToExcludeFromReference=None, logFile=None):
    ...
    #             gene = header.split('|')[6]
    #         elif header.startswith('>sp'):
    #             gene = header.split('=')[3].split()[0]
    #         else:
    #             print('Header file not from Gencode or swissprot')
    #             exit(1)
                
    #         if gene not in excludeGenes:
    #             if cleaveFile:
    #                 transcript_id = extractTranscriptId(header, 'pepsickle') # Indicating pepsickle transcript_ID
    #                 # TODO TEST
    #                 if transcript_id in cleaveDict:
    #                     ref_nmers.update(getCleavedMHCIPeptidesSet(orf, cleavageIndicesDict[transcript_id], pepLens))
    #                 else:
    ...
    #     if logFile:
    #         with open(logFile, 'w') as log:
    #             #log.write("Genes excluded from the reference proteome via GFF match:\n")
    #             for gene in sorted(foundGenes):
    #                 log.write(f"{gene}\n")
    # else:
    #     if cleaveFile:
    #         for header, orf in parseFasta(referenceFasta):
    #             transcript_id = extractTranscriptId(header, 'pepsickle') # Indicating pepsickle transcript_ID
    #             # TODO TEST
    #             if transcript_id in cleaveDict:
    #                 ref_nmers.update(getCleavedMHCIPeptidesSet(orf, cleavageIndicesDict[transcript_id], pepLens))
    ...
    #  if gff:
    #     ids = getIDsFromGff(genesToExcludeFromReference, gff)
    #     for header, prot in parseFasta(referenceFasta):
    #         if header.startswith('>ENSP'):
    #             tId = extractTranscriptId(header, 'referenceProteomeGencode')
    #         elif header.startswith('>sp'):
    #             tId = extractTranscriptId(header, 'referenceProteomeSwissprot')
    #         else:
    #             print('Header file not from Gencode or swissprot')
    #             exit(1)
    ...
    #             ref_nmers.update(getMHCIPeptidesSet(prot, pepLens))
    #         else:
    #             gene_name = header.split('|')[6]
    #             foundGenes.add(gene_name)
...
def buildPeptideTranscriptMap(inputFasta, pepLens=[8, 9, 10, 11], cleaveFile=None, threshold=1.0):
    ...
    # if cleaveFile:
    #     cleavageIndicesDict = getCleavageIndicesDict(cleaveFile, threshold)
    #     dictKeysSet = set(cleavageIndicesDict.keys())

    #     for header, orf in parseFasta(inputFasta):
    #         transcript_id = extractTranscriptId(header, 'pepsickle') # Indicating pepsickle transcript_ID
    #         # TODO TEST
    #         if transcript_id in dictKeysSet:
    #             for pep in getCleavedMHCIPeptidesSet(orf, cleavageIndicesDict[transcript_id], pepLens):
    #                 peptide_transcripts[pep].append(transcript_id)
    # else:
    #     for header, orf in parseFasta(inputFasta):
    #         transcript_id = extractTranscriptId(header, 'inputFasta')
...
def main():
    ...
    # TODO DEBUG
    # test_file = open('out/cleaved_nf_NoFilter.txt', 'w')
